...
{{- include "common.names.dependency.fullname" (dict "chartName" "postgres" "chartValues" .Values.postgres "context" .) -}}
{{- end -}}
...
{{- if .Values.postgres.enabled -}}
    {{- include "keycloak.postgres.fullname" . -}}
{{- else -}}
...
{{- if .Values.postgres.enabled }}
    {{- coalesce (((.Values.global).postgres).auth).database .Values.postgres.auth.database "postgres" -}}
{{- else -}}
...
{{- if .Values.postgres.enabled -}}
    {{- coalesce (((.Values.global).postgres).auth).username .Values.postgres.auth.username | default "" -}}
{{- else -}}
...
{{- ternary "public" .Values.externalDatabase.schema .Values.postgres.enabled -}}
{{- end -}}
...
{{- if .Values.postgres.enabled -}}
    {{- if not (empty (coalesce (((.Values.global).postgres).auth).existingSecret .Values.postgres.auth.existingSecret | default "")) -}}
        {{- tpl (coalesce (((.Values.global).postgres).auth).existingSecret .Values.postgres.auth.existingSecret) . -}}
    {{- else -}}
        {{- include "keycloak.postgres.fullname" . -}}
    {{- end -}}
...
{{- if .Values.postgres.enabled -}}
    {{- default "postgres-password" .Values.postgres.auth.secretKeys.adminPasswordKey -}}
...
keycloak: database
    ...
    Either deploy the PostgreSQL sub-chart (--set postgres.enabled=true),
